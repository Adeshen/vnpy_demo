...
CREATE_TRADE_TABLE_SCRIPT = f"""
dataPath = "{DB_PATH}"
db = database(dataPath)

// 定义表的列名和类型
trade_columns = ["symbol", "exchange", "tradeid", "side", "datetime", "price", "volume"]
trade_type = [SYMBOL, SYMBOL, STRING, BOOL, NANOTIMESTAMP, DOUBLE, DOUBLE]

// 创建表结构
tradeTable = table(1:0, trade_columns, trade_type)

// 创建分区表
db.createPartitionedTable(
    tradeTable,
    "trade",
    partitionColumns=["datetime"],  // 按时间分区
    sortColumns=["symbol", "exchange", "datetime"],  // 按 symbol、exchange 和 datetime 排序
    keepDuplicates=LAST  // 保留重复数据的最后一条
)
"""