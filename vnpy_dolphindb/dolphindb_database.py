...
class DolphindbDatabase(BaseDatabase):
    ...
    def __init__(self, setting = None) -> None:
        """构造函数"""
        if setting != None:
            self.user = setting["database.user"]
            self.password: str = setting["database.password"]
            self.host: str = setting["database.host"]
            self.port: int = setting["database.port"]
            self.db_path: str = "dfs://" + setting["database.database"]
        ...
        if not self.session.existsDatabase(self.db_path):
            ...
            self.session.run(CREATE_TRADE_TABLE_SCRIPT)
    ...
    def save_trade_data(self, trades):
        # 将 TradeData 转换为 DataFrame
        
        data = {
            "symbol": [trade.symbol for trade in trades],
            "exchange": [trade.exchange for trade in trades],
            "tradeid": [trade.tradeid for trade in trades],
            "side": [trade.side for trade in trades],
            "datetime": [np.datetime64(convert_tz(trade.datetime)) for trade in trades],
            "price": [trade.price for trade in trades],
            "volume": [trade.volume for trade in trades],
        }
    ...
    def load_trade_data(self, symbol, exchange, start: datetime, end: datetime):
        ...
        # end = np.datetime64(convert_tz(end))
